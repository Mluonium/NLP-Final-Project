...
MLM = 'distilbert-base-multilingual-cased'
BATCH_SIZE = 32
...
if __name__ == '__main__':
    ...
    print(label2id)
    print(id2label)
    train_labels = [label2id[label] for label_list in train_labels for label in label_list ]
    
    dev_text, dev_labels = myutils.read_data(sys.argv[2])
    dev_labels = [label2id[label] for label_list in dev_labels for label in label_list]
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx in range(0, len(train_text_batched)):
            ...
            # print(output_scores)
            # print(output_scores.shape)
            # print(train_labels_batched[batch_idx].shape)
            # flat_output_scores = output_scores.view(BATCH_SIZE* MAX_TRAIN_SENTS, -1)
            # flat_labels = train_labels_batched[batch_idx].view(BATCH_SIZE * MAX_TRAIN_SENTS)
            batch_loss = loss_function(output_scores, train_labels_batched[batch_idx])
            #batch_loss = loss_function(flat_output_scores, flat_labels)
    ...
    # Problems: makes a prediction for each sequence, train labels are somehow also a label per sequence ?? 
    # BATCHES 
    # convert to batches
